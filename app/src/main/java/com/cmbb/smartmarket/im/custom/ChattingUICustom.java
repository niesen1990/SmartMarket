package com.cmbb.smartmarket.im.custom;
...
/**
 * 聊天界面自带提供两种主题的自定义供用户方便的使用，用户可以通过{@link CustomHelper ｝中 实现 AdviceBinder.bindAdvice(PointCutEnum.CHATTING_FRAGMENT_UI_POINTCUT, ChattingUICustomSample.class);
...
public class ChattingUICustom extends IMChattingPageUI {
    ...
    public int getMsgBackgroundResId(YWConversation conversation, YWMessage message, boolean self) {
        ...
        if (msgType == YWMessage.SUB_MSG_TYPE.IM_TEXT || msgType == YWMessage.SUB_MSG_TYPE.IM_AUDIO) {
            if (self) {
                return R.drawable.im_talk_pop_r_bg;
            } else {
                return R.drawable.im_talk_pop_l_bg;
            }
        } else if (msgType == YWMessage.SUB_MSG_TYPE.IM_IMAGE) {
            if (self) {
                return R.drawable.im_talk_pic_pop_r_bg;
            } else {
                return R.drawable.im_talk_pic_pop_l_bg;
            }
...
//		return 0;
    ...
    *//**
    ...
    public Bitmap processBitmapOfLeftImageMsg(Bitmap input) {
        ...
        Bitmap distBitmap = YWIMImageUtils.getFromCacheOrDecodeResource(R.drawable.im_left_bubble);
    ...
    public Bitmap processBitmapOfRightImageMsg(Bitmap input) {
        Bitmap output = Bitmap.createBitmap(input.getWidth(),
                input.getHeight(), Bitmap.Config.ARGB_8888);
        //为提高性能，对取得的resource图片做缓存
        Bitmap distBitmap = YWIMImageUtils.getFromCacheOrDecodeResource(R.drawable.im_right_bubble);
        NinePatch np = new NinePatch(distBitmap, distBitmap.getNinePatchChunk(), null);
        Canvas canvas = new Canvas(output);
        final Paint paint = new Paint();
        final Rect rectSrc = new Rect(0, 0, input.getWidth(), input.getHeight());
...
//        }
    ...
    public View getCustomTitleView(final Fragment fragment,
        ...
        View view = inflater.inflate(R.layout.im_custom_chatting_title, new RelativeLayout(context), false);
        ...
        backView.setCompoundDrawablesWithIntrinsicBounds(R.drawable.im_common_back_btn_white, 0, 0, 0);
...
//                + "/alibaba/WXOPENI/云旺相册";
    ...
    public View getChattingFragmentCustomViewAdvice(Fragment fragment, Intent intent) {
        ...
        if (intent != null && intent.hasExtra("extraTribeId") && intent.hasExtra("conversationType")) {
            ...
            if (tribeId > 0 && conversationType == YWConversationType.Tribe.getValue()) {
                YWIMKit mIMKit = IMLoginHelper.getInstance().getIMKit();
                if (mIMKit == null) {
                    return null;
                }
                ...
                if (tribe != null && tribe.getMsgRecType() == YWProfileSettingsConstants.TRIBE_MSG_REJ) { //群在屏蔽的时候才显示。
                    ...
                    ViewGroup.MarginLayoutParams lp = new ViewGroup.MarginLayoutParams(
                            ...
                            (int) context.getResources().getDimension(R.dimen.im_hint_text_view_height));
                    lp.setMargins(0, (int) context.getResources().getDimension(R.dimen.im_title_bar_height), 0, 0);
    ...
    private void receiveTribeMsg(YWTribe tribe, final View view) {
        IMLoginHelper.getInstance().getIMKit().getIMCore().getTribeService().unblockTribe(tribe, new IWxCallback() {
            @Override
