package com.cmbb.smartmarket.im.contact;
...
public  class ContactProfileFragment extends Fragment implements OnClickListener{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mUserId = IMLoginHelper.getInstance().getIMKit().getIMCore().getLoginUserId();
	...
	private void addContactCacheUpdateListener() {
		if(mContactCacheUpdateListener!=null&& IMLoginHelper.getInstance().getIMKit()!=null)
			IMLoginHelper.getInstance().getIMKit().getContactService().addContactCacheUpdateListener(mContactCacheUpdateListener);
	}
	private void removeContactCacheUpdateListener() {
		if(mContactCacheUpdateListener!=null&& IMLoginHelper.getInstance().getIMKit()!=null)
			IMLoginHelper.getInstance().getIMKit().getContactService().removeContactCacheUpdateListener(mContactCacheUpdateListener);
	}
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        view = inflater.inflate(R.layout.im_fragment_contact_profile, null);
        init();
        return view;
    }
...
//		titleBar.setBackgroundColor(Color.parseColor("#00b4ff"));
	...
	private IYWContactService getContactService(){
		IYWContactService contactService = IMLoginHelper.getInstance().getIMKit().getContactService();
...
//			}
	...
	private void sendMessage(YWProfileInfo mYWProfileInfo) {
		if(mYWProfileInfo.userId.equals(IMLoginHelper.getInstance().getIMKit().getIMCore().getLoginUserId())) {
