...
import com.cmbb.smartmarket.im.FragmentTabs;
import com.cmbb.smartmarket.im.contact.ContactSystemMessageActivity;
import com.cmbb.smartmarket.im.tribe.TribeSystemMessageActivity;
...
public class ConversationListOperationCustom extends IMConversationListOperation {
    ...
    /**
     * 返回自定义会话和群会话的头像 url
     * 该方法只适用设置自定义会话和群会话的头像，设置单聊会话头像请参考{@link com.taobao.openimui.sample.UserProfileHelper}
     * @param fragment
...
//        WXAPI.getInstance().getConversationManager().markReaded(conversation);
        if (conversation.getConversationType() == YWConversationType.Custom) {
            ...
            if (conversationId.startsWith(FragmentTabs.SYSTEM_FRIEND_REQ_CONVERSATION)) {
                Intent intent = new Intent(BaseApplication.getContext(), ContactSystemMessageActivity.class);
                fragment.getActivity().startActivity(intent);
            }else if(conversationId.startsWith(FragmentTabs.SYSTEM_TRIBE_CONVERSATION)){
                Intent intent = new Intent(BaseApplication.getContext(), TribeSystemMessageActivity.class);
                fragment.getActivity().startActivity(intent);
            }
        }
